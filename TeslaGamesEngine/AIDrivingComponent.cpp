...
void AIDrivingComponent::Tick(float deltaTime)
{
	...
	if (abs(owner->GetForwardsSpeed()) < 1.0f && moved) {
		isStuck = true;
		lastPos = currentPos;
	}
	...
	if (isStuck) {
		owner->reverse(1.0f);
		if (sqrt(((currentPos.x - lastPos.x) * (currentPos.x - lastPos.x)) + ((currentPos.z - lastPos.y) * (currentPos.z - lastPos.y))) > 10.f) {
			isStuck = false;
			moved = false;
		}
	}
...
void AIDrivingComponent::MoveTo(DrivingTarget)
{
	...
	float forwardsMagnitude = (1.0f - turnMagnitude)/1.5;
